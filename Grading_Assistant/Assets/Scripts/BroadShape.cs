...
public class BroadShape : MonoBehaviour
{
    ...
    {
        buildExtrema();
        float pos = this.transform.position.x * 1000 + this.transform.position.y * 1000 + this.transform.position.z * 1000;
        rand = new System.Random((int)pos);
        mat = this.GetComponent<Renderer>().material;
        colorID = mat.shader.FindPropertyIndex("_Color");
        colorID = mat.shader.GetPropertyNameId(colorID);
        changeColor();
    }
    ...
    private void changeColor()
    {
        if (colorID >= 0)
        {
            ...
            float col = (float)(rand.Next(255)) / 255.0f;
            cols.x = col;
            col = (float)(rand.Next(255)) / 255.0f;
            cols.y = col;
            col = (float)(rand.Next(255)) / 255.0f;
            cols.z = col;
            cols.w = 1.0f;
            mat.SetVector(colorID, cols);
        }
    }
